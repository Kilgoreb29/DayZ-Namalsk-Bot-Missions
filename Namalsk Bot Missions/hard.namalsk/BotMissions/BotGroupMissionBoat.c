class BotGroupMissionBoat: CustomMission
{
	
	void BotGroupMissionBoat() {}
	
	
	// Конфиги ботов
	vector BotSpawnPoint = "3951.092041 10.422597 10250.902344";				// задаем точку спавна бота	
	...
	protected int botLootCountMax = 4; 										// назначаем максимальное количество лута у бота
	...
	void AddCeckPoint(SurvivorBotBase m_BotSolder)
	{
		...
		m_BotSolder.AddCheckpoint("3935.497803 9.866616 10262.967773");
		m_BotSolder.AddCheckpoint("3935.497803 9.866616 10262.967773");
		m_BotSolder.AddCheckpoint("3935.497803 9.866616 10262.967773");
		m_BotSolder.AddCheckpoint("3935.497803 9.866616 10262.967773");
		m_BotSolder.AddCheckpoint("3935.497803 9.866616 10262.967773");
	...
	void createBotUnit()
	{
		...
		if (m_BotSolder.IsAlive() && m_BotSolder && onRespawnBot)
			GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(respawnBotUnitC4, m_respawnTime * 60000, true, m_BotSolder);
	}
	// ----------------------------- Конец функции спавна бота ------------------------------------- //
	
	// ----------------------------- Функция респавна ботов ----------------------------------------//
	private void respawnBotUnitC4(SurvivorBotBase m_Bot)
	{
	...
	{  
		...
		{	
			for ( int u = 0; u < players.Count(); u++ )
			{
				...
				{
					spawnBotGroup();
					GetGame().GetCallQueue(CALL_CATEGORY_GAMEPLAY).Remove(TriggerPlayersC4);			
				} 
			}
		}
	} 
	...
	void StartMissionAI()
	{
		...
		if (canUseTrigger)
		{
			GetGame().GetCallQueue(CALL_CATEGORY_GAMEPLAY).CallLater(TriggerPlayersC4, 5000, true);			
		}
		else
		{
			spawnBotGroup();
